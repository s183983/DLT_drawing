# -*- coding: utf-8 -*-
"""
Created on Tue Sep  6 08:31:21 2022
...
class Annotator(PyQt5.QtWidgets.QWidget):
    ...
    def __init__(self, size=None, resize_size = 128):
        ...
        self.resizePix128 = PyQt5.QtGui.QPixmap(128, 128)
        self.resizePix128.fill(self.color_picker(label=0, opacity=0))
        self.resizePix256 = PyQt5.QtGui.QPixmap(256, 256)
        self.resizePix256.fill(self.color_picker(label=0, opacity=0))
        ...
        self.oldAnn= [self.annotationPix.copy()]
        self.oldResize128,self.oldResize256 = [self.resizePix128.copy()],[self.resizePix256.copy()]
    ...
    def fromFileList(cls, filelist, label_path, save_path):
        ...
        resize_shape = (int(im.shape[1]*scale_factor),int(im.shape[1]*scale_factor))
    ...
    def reset_image(self):
        ...
        # self.device = device
    def showLabel(self):
        self.label_name = os.path.join(self.label_path, os.path.basename(self.file_name).split('.')[0]+".png")
        label = np.array(Image.open(self.label_name))
    ...
    def mousePressEvent(self, event):
        if event.button() == PyQt5.QtCore.Qt.LeftButton: 
            if self.zPressed: # initiate zooming and not drawing
                ...
                painter_resize128 = self.makePainter_resize(self.resizePix128, 
                        self.color_picker(self.label, 
                            (self.label>0)*self.annotationOpacity)) # the painter used for drawing 
                point128 = PyQt5.QtCore.QPoint((event.x()-self.padding.x())/self.target.width() * 128,
                                    (event.y()-self.padding.y())/self.target.height() * 128)
                painter_resize128.drawPoint(point128)
                
                painter_resize256 = self.makePainter_resize(self.resizePix256, 
                        self.color_picker(self.label, 
                            (self.label>0)*self.annotationOpacity)) # the painter used for drawing 
                point256 = PyQt5.QtCore.QPoint((event.x()-self.padding.x())/self.target.width() * 256,
                                    (event.y()-self.padding.y())/self.target.height() * 256)
                painter_resize256.drawPoint(point256)
                
                self.last_resize_point128 = point128
                self.last_resize_point256 = point256
    ...
    def mouseMoveEvent(self, event):
        if self.activelyZooming: 
            ...
            if self.activelyDrawing: 
                ...
                painter_resize128.drawLine(self.last_resize_point128, point128)
                painter_resize256.drawLine(self.last_resize_point256, point256)
                self.last_resize_point128 = point128
                self.last_resize_point256 = point256
                # self.point_list.append([point.x(), point.y(), self.label])
    ...
    def draw_masks(self,masks, un_col): 
        ...
        colors = self.annotationColor
        
        n_masks = min(len(un_col),len(colors)-1)
    ...
    def undo(self):
        ...
        if len(self.oldAnn)>1 and len(self.oldResize128)>1:
            self.oldAnn.pop()
            self.oldResize128.pop()
            self.oldResize256.pop()
            # print("undo")
            self.annotationPix = self.oldAnn[-1].copy()
            self.resizePix128 = self.oldResize128[-1].copy()
            self.resizePix256 = self.oldResize256[-1].copy()
    ...
    def reset_current_image(self):
        self.annotationPix = self.oldAnn[0].copy()
        self.resizePix128 = self.oldResize128[0].copy()
        self.resizePix256 = self.oldResize256[0].copy()
        self.oldAnn = [self.oldAnn[0]] 
        self.oldResize128 = [self.oldResize128[0]]
        self.oldResize256 = [self.oldResize256[0]]
